...
function getRectangleSegments() {
    ...
    return [
        ...
        [ left - halfLineWidth,            top + halfLineWidth + height         ],
...
function strokeAnimationSegments(animation, segments, fraction) {
    ...
    for(let index = 1; index < segments.length; index++) {
        ...
        context.lineTo(start[0] + (difference[0] * fraction), start[1] + (difference[1] * fraction));
    }
};
...
const animationDuration = animations.map((animation) => animation.duration).reduce((a, b) => a + b);
